...
// ─── Цифры, которые меняются на глазах ─────────────────────────────
//
// В обычном шрифте цифры разной ширины: «1» уже «8». Пока текст статичен, это
// незаметно и даже лучше читается. Но там, где число живёт — секундомер
// тренировки тикает раз в секунду, баллы пересчитываются после каждой отметки,
// место в рейтинге меняется, — строка дёргается на каждом обновлении: соседние
// подписи ползают влево-вправо, и глаз цепляется за движение вместо цифры.
//
// Табличные цифры дают всем десяти одинаковый шаг: значение меняется, ширина —
// нет. Ставится ТОЧЕЧНО, только на живые и колоночные числа; в обычном тексте
// пропорциональные цифры остаются как были.
export const nums: TextStyle = { fontVariant: ['tabular-nums'] };
