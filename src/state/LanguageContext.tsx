...
export function LanguageProvider({ children }: { children: React.ReactNode }) {
  // Pinned to Russian. setLang is a no-op kept for call-site compatibility.
  const lang: Lang = 'ru';
  _lang = 'ru';
  const setLang = useCallback((_l: Lang) => {}, []);
  const t = useCallback((k: keyof typeof STR) => (STR[k] ? STR[k].ru : String(k)), []);
  const tr = useCallback((ru: string) => ru, []);
  const value = useMemo(() => ({ lang, setLang, t, tr }), [setLang, t, tr]);
