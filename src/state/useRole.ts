...
export function useRole() {
  ...
  useEffect(() => {
    let alive = true;
    (async () => {
      if (!isSignedIn) { setCanCreate(false); setIsAdmin(false); setEmail(null); return; }
      ...
      for (let attempt = 0; attempt < 3 && alive; attempt++) {
        try {
          ...
          if (alive) { setCanCreate(!!r.canCreate); setIsAdmin(!!r.isAdmin); setEmail(r.email ?? null); }
          return;
        } catch {
  ...
  return { canCreate, isAdmin, email };
}
